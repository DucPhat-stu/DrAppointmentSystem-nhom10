...
- [ ] ai-service ❌ **NOT CREATED** (Task for Sprint 1)
...
- [ ] ChatbotPage ❌ **NOT CREATED** (Task for Sprint 1)

### Database
- [x] PostgreSQL running ✅
- [x] Docker Compose ready ✅
- [ ] ai_service DB tables ❌ (Will create in Story 1)
...
- [ ] Create directory: `backend/services/ai-service`
- [ ] Create `pom.xml` with dependencies:
  ...
  - lombok
  - jackson
- [ ] Copy structure from appointment-service:
...
- [ ] Create `infrastructure/AIClient.java`
- [ ] Implement HTTP call to Gemini API
- [ ] Add timeout configuration (2s)
- [ ] Implement retry logic (1 retry on timeout)
- [ ] Error handling (return error response, no exception thrown)
...
- [ ] Create tables (if any needed for Sprint 1)
- [ ] Run migration locally

#### Task 1.6: Add to Backend Parent POM
- [ ] Update `backend/pom.xml` to include ai-service module
...
- [ ] Create `dto/AICheckResponse.java`:
  ```java
...
- [ ] Create `application/AIResponseParser.java`
...
- [ ] Test missing fields (returns fallback)
- [ ] Test validation rules (boundary cases)
...
- [ ] Create `application/AITextFormatter.java`
...
- [ ] Null values → use "Không xác định"
...
- [ ] Create `dto/AICheckRequest.java`:
  ```java
...
- [ ] Endpoint: `POST /api/v1/ai/check`
- [ ] Input validation using @Valid
- [ ] Call AIClient → Parser → Formatter pipeline
- [ ] Return formatted text in ApiResponse<String>
...
- [ ] Create Postman request for POST /api/v1/ai/check
- [ ] Add example with valid input
- [ ] Add example with error input
- [ ] Add to healthcare-local environment
...
- [ ] Implement `checkSymptoms(text)` function
- [ ] Use httpClient.post('/ai/check', {text})
...
- [ ] Placeholder text: "Mô tả triệu chứng của bạn..."
...
#### Task 5.5: CSS Module
- [ ] Create `pages/ChatbotPage.module.css`
- [ ] Styling:
...
- [ ] Disable input box during request
- [ ] Show "Đang xử lý..." on send button
- [ ] Add timeout warning after 3s (if not responded)

#### Task 6.2: Error Messages
- [ ] Create error messages for different scenarios:
...
- [ ] Create error box with:
  - ⚠️ Icon
  - Error message
  - Retry button (optional)

#### Task 6.4: Prevent Spam
- [ ] Add 2s cooldown between requests
- [ ] Track last request time
- [ ] Disable send button if within cooldown
